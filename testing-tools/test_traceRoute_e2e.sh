...
test_script=test_traceRoute.sh

if [[ $1 == "-h"  ]]; then
  echo "[Help] Please enter the command: ./test_traceRoute_e2e.sh {source_server} {ssh_id} {ssh_pw} {target_server}"
  echo "[Help] example: ./test_traceRoute_e2e.sh 127.0.0.1 ubuntu_id ubuntu_pw 8.8.8.8"
else
